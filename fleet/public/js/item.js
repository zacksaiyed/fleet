...
frappe.ui.form.on('Item', {
    ...
    custom_model(frm) {
        ...
        frappe.db.get_value(
            ...
            if (r && r.message) {
                ...
                console.log("custom_default_billing_price set to", r.message.price);
            }
        });
    },
    ...
    // ==========================================
    // Lumpsum & Maintain Stock Validations
    // ==========================================
    validate: function(frm) {
        if (frm.doc.is_stock_item === 1 && frm.doc.custom_is_lumpsum_amount_item === 1) {
            frappe.msgprint({
                title: __('Validation Error'),
                indicator: 'red',
                message: __('You cannot enable "Is Lumpsum Amount Item" when "Maintain Stock" is checked.')
            });
            frm.validated = false;
        }
    },
    ...
    is_stock_item: function(frm) {
        if (frm.doc.is_stock_item === 1 && frm.doc.custom_is_lumpsum_amount_item === 1) {
            frm.set_value('custom_is_lumpsum_amount_item', 0);
            frappe.show_alert({
                message: __('"Is Lumpsum Amount Item" cannot be enabled when "Maintain Stock" is checked.'),
                indicator: 'orange'
            });
        }
    },

    custom_is_lumpsum_amount_item: function(frm) {
        if (frm.doc.is_stock_item === 1 && frm.doc.custom_is_lumpsum_amount_item === 1) {
            frm.set_value('custom_is_lumpsum_amount_item', 0);
            frappe.msgprint({
                title: __('Validation Error'),
                indicator: 'red',
                message: __('You cannot enable "Is Lumpsum Amount Item" while "Maintain Stock" is enabled.')
            });
            return;
        }

        if (frm.doc.custom_is_lumpsum_amount_item === 1) {
            frappe.db.get_list('Item', {
                filters: [
                    ['custom_is_lumpsum_amount_item', '=', 1],
                    ['name', '!=', frm.doc.name]
                ],
                fields: ['name'],
                limit: 1
                ...
                if (records && records.length > 0) {
                    frappe.msgprint({
                        title: __('Validation Error'),
                        indicator: 'red',
                        message: __('Another item (' + records[0].name + ') is already marked as the Lumpsum Amount Item. Only one item can have this option enabled.')
                    });
                    frm.set_value('custom_is_lumpsum_amount_item', 0);
                }
            });
